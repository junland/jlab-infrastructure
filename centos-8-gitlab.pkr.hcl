...
source "qemu" "autogenerated_1" {
  accelerator      = "kvm"
  boot_command     = ["<up><wait><tab><wait> text inst.ks=http://{{ .HTTPIP }}:{{ .HTTPPort }}/centos-kvm-ks.cfg<enter><wait>"]
  boot_wait        = "10s"
...
build {
  ...
  provisioner "shell" {
    ...
    pause_before     = "10s"
    scripts          = ["scripts/centos/00-base.sh", "scripts/centos/99-gitlab-runner.sh"]
  }
